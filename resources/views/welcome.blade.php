{{-- <!DOCTYPE html>
<html lang="en">

<head>
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" />
    <meta charset="utf-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />

    <title>Instagress Alternative — Instagram bot for like, comment, follow, unfollow</title>

    <meta name="description" content="The best online Instagram bot on the web. Automate your activity and get more followers, likes and comments on Instagram." />

    <meta name="keywords" content="instagram get more followers, instagram bot, instagram bot online, get instagram followers" />

    <!-- Facebook open graph tags -->
    <meta property="og:type" content="website" />
    <meta property="og:site_name" content="Instagress" />
    <meta property="og:url" content="https://instagressalternative.com" />
    <meta property="og:title" content="Instagress — Best Instagram Automation Service" />
    <meta property="og:description" content="Automate your activity and get more real followers, likes and comments on Instagram." />
    <meta property="og:image" content="https://instagressalternative.com/img/share-img.png?v=203" />

    <!-- Twitter card tags -->
    <meta name="twitter:card" content="summary_large_image" />
    <meta name="twitter:site" content="@instagress" />
    <meta name="twitter:title" content="Instagress — Best Instagram Automation Service" />
    <meta name="twitter:description" content="Automate your activity and get more real followers, likes and comments on Instagram." />
    <meta name="twitter:image" content="https://instagressalternative.com/img/share-img.png?v=203" />

    <!-- Favicon & site image -->
    <link rel="shortcut icon" type="image/png" href="favicon-v=203.png" />
    <link rel="icon" type="image/png" href="favicon-v=203.png" />
    <link rel="image_src" type="image/png" href="{{ asset('img/share-img-v=203.png') }}" />

    <!-- Apple touch icons -->
    <link rel="apple-touch-icon-precomposed" sizes="152x152" href="{{ asset('img/apple-touch-icon-152x152-v=203.png') }}" />
    <link rel="apple-touch-icon-precomposed" sizes="144x144" href="{{ asset('img/apple-touch-icon-144x144-v=203.png') }}" />
    <link rel="apple-touch-icon-precomposed" sizes="120x120" href="{{ asset('img/apple-touch-icon-120x120-v=203.png') }}" />
    <link rel="apple-touch-icon-precomposed" sizes="114x114" href="{{ asset('img/apple-touch-icon-114x114-v=203.png') }}" />
    <link rel="apple-touch-icon-precomposed" sizes="76x76" href="{{ asset('img/apple-touch-icon-76x76-v=203.png') }}" />
    <link rel="apple-touch-icon-precomposed" sizes="72x72" href="{{ asset('img/apple-touch-icon-72x72-v=203.png') }}a" />
    <link rel="apple-touch-icon-precomposed" sizes="57x57" href="{{ asset('img/apple-touch-icon-57x57-v=203.png') }}" />
    <link rel="apple-touch-icon-precomposed" href="{{ asset('img/apple-touch-icon-60x60-v=203.png') }}" />

    <!-- Fonts -->

    <link rel="stylesheet" href="https://fonts.googleapis.com/css?family=Open+Sans:400,600,700" />

    <!-- Styles -->

    <link rel="stylesheet" href="css/styles.bundle-v=203.css" media="all" />

    <!-- Scripts -->

    <script src="js/scripts.bundle-v=203.js"></script>

    <script type="text/javascript">
        var _gaq = _gaq || [];
        _gaq.push(['_setAccount', 'UA-43454158-1']);
        _gaq.push(['_trackPageview']);

        (function() {
            var ga = document.createElement('script');
            ga.type = 'text/javascript';
            ga.async = true;
            ga.src = ('https:' == document.location.protocol ? 'https://ssl' : 'http://www') + '.google-analytics.com/ga.js';
            var s = document.getElementsByTagName('script')[0];
            s.parentNode.insertBefore(ga, s);
        })();
    </script>

    <!-- HTML5 shim and Respond.js for IE8 support of HTML5 elements and media queries -->
    <!-- WARNING: Respond.js doesn't work if you view the page via file:// -->
    <!--[if lt IE 9]>
<script src="https://oss.maxcdn.com/html5shiv/3.7.2/html5shiv.min.js"></script>
<script src="https://oss.maxcdn.com/respond/1.4.2/respond.min.js"></script>
<![endif]-->

</head>

<body>
    <header>

    <footer>
        <div class="container">
            <div class="row">
                <div class="col-md-3">
                    <a href="index.html" class="logo-footer-new">
                        <img src="img/instagress-logo-footer-v=203.png" srcset="/img/instagress-logo-footer-2x.png?v=203 2x" alt="Instagress" />
                    </a>
                    <div class="copyright">&copy; 2013-2016</div>
                </div>
                <div class="col-md-6 menu-cont">
                    <ul>

                        <li class="">
                            <a href="about.html">About</a>

                        </li>

                        <li class="">
                            <a href="prices.html">Prices</a>

                        </li>

                        <li class="">
                            <a href="blog.html">Blog</a>

                        </li>

                        <li class="more">
                            <a href="guide.html">Guide</a>

                        </li>

                        <li class="more">
                            <a href="faq.html">FAQ</a>

                        </li>

                        <li class="">
                            <a href="index.html#" data-popup-open="#popup-support">Support</a>

                        </li>

                        <li class="toggle-more">
                            <a href="index.html#" class="btn-navbar-menu-more" data-ui-menu-toggle="navbar-menu-more" data-ui-role="dropdown"></a>
                            <div class="navbar-popup-menu" data-ui="navbar-menu-more">
                                <ul>

                                    <li class="">
                                        <a href="guide.html">Guide</a>

                                    </li>

                                    <li class="">
                                        <a href="faq.html">FAQ</a>

                                    </li>

                                </ul>

                            </div>
                        </li>
                    </ul>

                    <ul class="menu-cont-second">

                        <li class="">
                            <a href="terms.html">Terms of Service</a>

                        </li>

                        <li class="">
                            <a href="privacy.html">Privacy Policy</a>

                        </li>

                    </ul>
                </div>
                <div class="col-md-3">
                    <div class="social-icons clearfix">
                        <a href="https://www.facebook.com/instagress" class="mr5" target="_blank" title="Follow us on Facebook">
                            <i class="fa fa-facebook"></i>
                        </a>
                        <a href="https://twitter.com/instagress" class="mr5" target="_blank" title="Follow us on Twitter">
                            <i class="fa fa-twitter"></i>
                        </a>
                        <a href="https://plus.google.com/+InstagressApp" class="mr5" target="_blank" title="Follow us on Google+">
                            <i class="fa fa-google-plus"></i>
                        </a>
                        <a href="http://instagress.tumblr.com/" target="_blank" title="Follow us on Tumblr">
                            <i class="fa fa-tumblr"></i>
                        </a>
                    </div>
                    <div class="support">
                        <a href="mailto:support@instagressalternative.com">support@instagressalternative.com</a>
                    </div>
                </div>
            </div>
        </div>
    </footer>

    <div id="popup-alert" class="popup popup-small">
        <a href="index.html#" class="btn-close-x" data-popup-close="#popup-alert"></a>
        <div class="text"></div>
    </div>

    <div id="popup-support" class="popup">
        <h3>Support</h3>
        <a href="index.html#" class="btn-close-x" data-popup-close="#popup-support"></a>
        <div class="color-gray mb20">
            Please use the form below if you have any questions, comments or would like to give us some feedback. Your opinion is very important to us!
        </div>
        <div class="alert alert-info">
            If you have a question or you want to ask about an error, please try to find an answer on the <a href="faq.html">FAQ</a> page by yourself at first.
        </div>
        <div class="alerts">
            <div class="alert alert-danger alert-error"></div>
            <div class="alert alert-success"></div>
        </div>
        <script type="text/javascript">
            function sendmail() {
                _data = $(this).serialize();
                $.post("sendmail.php", _data, function(result) {
                    console.log(result);
                    alert(result);
                });
            };
        </script>
        <form class="form-horizontal form-ajax2 mt20" method="post">
            <input type="hidden" name="token" value="" />
            <div class="form-group field-wrap" data-field="subject">
                <div class="col-md-12">
                    <label class="control-label text-align-left" for="inpSupportSubject">Subject:</label>
                    <select name="subject" id="inpSupportSubject" class="form-control" autofocus>

                        <option value="question">Question</option>

                        <option value="error">Error</option>

                        <option value="idea">Idea</option>

                        <option value="payment">Payment</option>

                        <option value="refund">Refund</option>

                        <option value="discount">Discount</option>

                        <option value="security">Security</option>

                        <option value="other">Other</option>

                    </select>
                    <span class="help-block text-danger text-error hidden"></span>
                </div>
            </div>
            <div class="form-group field-wrap" data-field="email">
                <div class="col-md-12">
                    <label class="control-label text-align-left" for="inpSupportEmail">Email:</label>

                    <input type="text" name="email" id="inpSupportEmail" class="form-control" placeholder="Your email" value="" />
                    <span class="help-block text-danger text-error hidden"></span>
                </div>
            </div>
            <div class="form-group field-wrap" data-field="username">
                <div class="col-md-12">
                    <label class="control-label text-align-left" for="inpSupportUsername">Username:</label>
                    <input type="text" name="username" id="inpSupportUsername" class="form-control" placeholder="Your Instagram username (optional)" value="" />
                    <span class="help-block text-danger text-error hidden"></span>
                </div>
            </div>
            <div class="form-group field-wrap" data-field="message">
                <div class="col-md-12">
                    <label class="control-label text-align-left" for="inpSupportMessage">Message:</label>
                    <textarea name="message" id="inpSupportMessage" rows="4" class="form-control" placeholder="Your message"></textarea>
                    <span class="help-block text-danger text-error hidden"></span>
                </div>
            </div>
            <div>
                <button type="button" onclick="sendmail();" class="btn btn-plain btn-success" data-loading-text="Sending...">Send</button>
            </div>
        </form>
    </div>

    

    <div id="popup-login-help" class="popup">
        <h3>Sign in Help</h3>
        <a href="index.html#" class="btn-close-x" data-popup-close="#popup-login-help"></a>
        <div class="mb20">
            <p>
                Having trouble logging in with your Instagram credentials? These can be the possible reasons:
            </p>
            <ul class="nice-list">
                <li>
                    <p><strong>Wrong username or password</strong></p>
                    <p>
                        Check that you have not misspelled your username or password. Please, note that you should enter your username without @ symbol before it.
                    </p>
                </li>
                <li>
                    <p><strong>Integrity checkpoint</strong></p>
                    <p>
                        This error means that Instagram is asking you to verify your account. You must be logged in on Instagram website with your Instagram account to do it.
                    </p>
                </li>
                <li>
                    <p><strong>Instagram login error</strong></p>
                    <p>
                        It looks like Instagram requires to additionally verify your account. There could be several verification options. Please, login on Instagram website using your credentials to complete additional verification.
                    </p>
                </li>
                <li>
                    <p><strong>Instagram password reset</strong></p>
                    <p>
                        In rare cases Instagram can reset your password when you're trying to login on third-party website. Go to your email (associated with your Instagram account) and check your inbox/spam folders for a message from Instagram with password reset link.
                    </p>
                </li>
            </ul>
            <p>
                If you will continue to receive any login errors, please
                <a href="index.html#" class="link-ajax" data-popup-open="#popup-support">contact&nbsp;us</a>.
            </p>
        </div>
        <div class="text-align-center">
            <button class="btn btn-plain" data-popup-close="#popup-login-help">Close</button>
        </div>
    </div>

    <div id="popup-reauth" class="popup">
        <h3>Account Reconnect</h3>
        <a href="index.html#" class="btn-close-x" data-popup-close="#popup-reauth"></a>
        <div class="alerts">
            <div class="alert alert-danger alert-error"></div>
            <div class="alert alert-success"></div>
        </div>
        <form action="/auth/login?mode=reauth&next=/" id="reauth-form" class="form-horizontal form-ajax mb0 js-ig-auth-form" method="post">
            <div class="form-group field-wrap" data-field="username">
                <div class="col-md-12">
                    <input type="text" name="username" id="inpReauthUsername" class="form-control input-icon input-icon-username" placeholder="Instagram username" />
                    <span class="help-block text-danger text-error hidden"></span>
                </div>
            </div>
            <div class="form-group field-wrap" data-field="password">
                <div class="col-md-12">
                    <input type="password" name="password" id="inpReauthPassword" class="form-control input-icon input-icon-password" placeholder="Instagram password" autofocus/>
                    <span class="help-block text-danger text-error hidden"></span>
                </div>
            </div>
            <div>
                <input type="hidden" name="ap1" value="" />
                <input type="hidden" name="ap2" value="" />
                <button type="submit" class="btn btn-plain btn-success mb20" data-loading-text="Signing in...">Sign in</button>
            </div>
            <div>
                <a href="index.html#" class="link-ajax" data-popup-open="#popup-login-help">Sign in help</a>
            </div>
        </form>
    </div>

    <div id="popup-account-signup" class="popup">
        <h3>Sign up</h3>
        <a href="index.html#" class="btn-close-x" data-popup-close="#popup-account-signup"></a>

        <div class="alerts">
            <div class="alert alert-danger alert-error"></div>
            <div class="alert alert-success"></div>
        </div>
        <form action="/account/signup" id="account-signup-form" class="form-horizontal form-ajax mb0" method="post">

            <div class="row mb20">
                <div class="col-sm-6 pr10-sm-min">
                    <a href="https://instagressalternative.com/account/signin/fb" class="btn btn-plain btn-fb btn-responsive">
                        <i class="fa fa-facebook-square fa-lg mr5"></i> Sign up with Facebook
                    </a>
                </div>
                <div class="col-sm-6 pl10-sm-min mt15-xs-max">
                    <a href="https://instagressalternative.com/account/signin/gl" class="btn btn-plain btn-gl btn-responsive">
                        <i class="fa fa-google-plus-square fa-lg mr5"></i> Sign up with Google
                    </a>
                </div>
            </div>
            <div class="dividing-line dividing-line-popup">
                <hr>
                <span>or</span>
            </div>

            <div class="form-group field-wrap" data-field="email">
                <div class="col-md-12">
                    <input type="email" name="email" id="inpAccountSignupEmail" class="form-control input-icon input-icon-email" placeholder="Email" autofocus/>
                    <span class="help-block text-danger text-error hidden"></span>
                </div>
            </div>
            <div class="form-group field-wrap" data-field="password">
                <div class="col-md-12">
                    <input type="password" name="password" id="inpAccountSignupPassword" class="form-control input-icon input-icon-password" placeholder="Password (random by default)" />
                    <span class="help-block text-danger text-error hidden"></span>
                </div>
            </div>
            <div>
                <button type="submit" class="btn btn-plain btn-success mb20" data-loading-text="Signing up...">Sign up</button>
            </div>
            <div class="mb10">
                By signing up, you agree to the <a href="terms.html">Terms of Service</a>.
            </div>
            <div>
                Already have an account?
                <a href="index.html#" class="link-ajax" data-popup-close="#popup-account-signup" data-popup-open="#popup-account-login">Log in</a>
            </div>

        </form>
    </div>

    <div id="popup-account-login" class="popup">
        <h3>Log in</h3>
        <a href="index.html#" class="btn-close-x" data-popup-close="#popup-account-login"></a>
        <div class="alerts">
            <div class="alert alert-danger alert-error"></div>
            <div class="alert alert-success"></div>
        </div>
        <form action="/account/login" id="account-login-form" class="form-horizontal form-ajax mb0" method="post">

            <div class="row mb20">
                <div class="col-sm-6 pr10-sm-min">
                    <a href="https://instagressalternative.com/account/signin/fb" class="btn btn-plain btn-fb btn-responsive">
                        <i class="fa fa-facebook-square fa-lg mr5"></i> Log in with Facebook
                    </a>
                </div>
                <div class="col-sm-6 pl10-sm-min mt15-xs-max">
                    <a href="https://instagressalternative.com/account/signin/gl" class="btn btn-plain btn-gl btn-responsive">
                        <i class="fa fa-google-plus-square fa-lg mr5"></i> Log in with Google
                    </a>
                </div>
            </div>
            <div class="dividing-line dividing-line-popup">
                <hr>
                <span>or</span>
            </div>

            <div class="form-group field-wrap" data-field="email">
                <div class="col-md-12">
                    <input type="email" name="email" id="inpAccountLoginEmail" class="form-control input-icon input-icon-email" placeholder="Email" autofocus/>
                    <span class="help-block text-danger text-error hidden"></span>
                </div>
            </div>
            <div class="form-group field-wrap" data-field="password">
                <div class="col-md-12">
                    <input type="password" name="password" id="inpAccountLoginPassword" class="form-control input-icon input-icon-password" placeholder="Password" />
                    <span class="help-block text-danger text-error hidden"></span>
                </div>
            </div>
            <div>
                <button type="submit" class="btn btn-plain btn-success mb20" data-loading-text="Logging in...">Log in</button>
            </div>
            <div class="mb10">
                <a href="index.html#" class="link-ajax" data-popup-open="#popup-account-password-reset">Password reset</a>
            </div>
            <div>
                Don't have an account?
                <a href="index.html#" class="link-ajax" data-popup-close="#popup-account-login" data-popup-open="#popup-account-signup">Sign up</a>
            </div>

        </form>
    </div>

    <div id="popup-account-password-reset" class="popup">
        <h3>Password Reset</h3>
        <a href="index.html#" class="btn-close-x" data-popup-close="#popup-account-password-reset"></a>
        <div class="color-gray mb20">
            If you forgot your password, request a new one by entering your email address in this form.
        </div>
        <div class="alerts">
            <div class="alert alert-danger alert-error"></div>
            <div class="alert alert-success"></div>
        </div>
        <form action="/account/password/reset" id="account-password-reset-form" class="form-horizontal form-ajax mb0" method="post">
            <div class="form-group field-wrap" data-field="email">
                <div class="col-md-12">
                    <input type="email" name="email" id="inpAccountPasswordResetEmail" class="form-control input-icon input-icon-email" placeholder="Email" value="" autofocus/>
                    <span class="help-block text-danger text-error hidden"></span>
                </div>
            </div>
            <div>
                <button type="submit" class="btn btn-plain btn-success" data-loading-text="Sending request...">Send request</button>
            </div>
        </form>
    </div>

    <div id="popup-account-required" class="popup">
        <h3>Dashboard Account</h3>
        <a href="index.html#" class="btn-close-x" data-popup-close="#popup-account-required"></a>
        <p>
            Please, create your personal Instagress Dashboard account because we've switched to a new login system to provide better services and continue to integrate more new features.
        </p>
        <p>
            You will be able to add all your Instagram usernames with your time, settings and all other data to your personal Instagress account.

        </p>
        <p>
            With Instagress account you will be able to use our most notable features:
            <a href="blog/dashboard.html">Dashboard for multiple usernames</a>,
            <a href="blog/the-new-payment-system.html">New Payment System</a>,
            <a href="blog/quantity-discount.html">Quantity Discount</a>,
            <a href="blog/time-transfer.html">Time Transfer</a>,
            <a href="blog/protected-time-transfer.html">Protected Time Transfer</a> and more other new and upcoming features.
        </p>
        <p>
            It's completely FREE and easy: just use your email or sign up with Facebook or Google.
        </p>
        <div class="text-align-center mt30">

            <div class="row mb20">
                <div class="col-sm-6 pr10-sm-min">
                    <a href="https://instagressalternative.com/account/signin/fb" class="btn btn-plain btn-fb btn-responsive">
                        <i class="fa fa-facebook-square fa-lg mr5"></i> Sign up with Facebook
                    </a>
                </div>
                <div class="col-sm-6 pl10-sm-min mt15-xs-max">
                    <a href="https://instagressalternative.com/account/signin/gl" class="btn btn-plain btn-gl btn-responsive">
                        <i class="fa fa-google-plus-square fa-lg mr5"></i> Sign up with Google
                    </a>
                </div>
            </div>
            <div class="dividing-line dividing-line-popup">
                <hr>
                <span>or</span>
            </div>

            <div class="row">
                <div class="col-sm-6 col-sm-offset-3">
                    <button class="btn btn-plain btn-success btn-responsive" data-popup-close="#popup-account-required" data-popup-open="#popup-account-signup">Sign up with email</button>
                </div>
            </div>
        </div>
    </div>

</body>

</html> --}}




@extends('layouts.main')

@section('content')

    <div class="red-line"></div>

    <section class="index">
    <div class="hero">
        <div class="container">
            <h1>Get social media superpowers</h1>
            <p class="description">Accelerate your life on Instagram for more targeted likes, comments and follows</p>
            <div class="row">
                <div class="col-sm-6">
                    <a href="http://www.instagressalternative.com/app/" class="btn btn-plain btn-big btn-main btn-danger btn-fw-xs-max mt20 float-right-sm-min">
                        Buy now
                    </a>
                </div>
                <div class="col-sm-6">
                    <a href="{{ route('activity.index') }}" @guest data-popup-open="#popup-account-login" @endguest   class="btn btn-plain btn-big btn-border btn-main btn-fw-xs-max mt20 float-left-sm-min">
                        Login
                    </a>
                </div>
                <!--
                <div class="text-large mt10">
                    <a href="#" class="link-ajax" data-popup-open="#popup-account-login">Log in</a>
                </div>
                -->
            </div>
            <div class="hero-devices">
                <div class="device-iphone"></div>
                <div class="device-ipad"></div>
                <div class="device-macbook"></div>
            </div>
        </div>
    </div>

    <!--
    <a href="/activity/demo" class="index-small-block">
    <div class="container">
        <div class="small-block-look-inside">
            <h4>Look inside</h4>
        </div>
    </div>
    </a>
    -->

    <div class="index-our-benefits">
        <div class="container">
            <h2 class="title-big">Our Benefits</h2>
            <p class="description">Why Instagress is the best service for Instagram automation</p>
            <div class="row">
                <div class="col-sm-4">
                    <div class="i-box">
                        <div class="icons icon-planet">
                        </div>
                        <h3>No downloads</h3>
                        <p>
                            You can use Instagress straight from the web on all browsers. You don't need to download or install anything to enjoy our service, which is why Instagress is the safest Instagram bot available.
                        </p>
                    </div>
                </div>
                <div class="col-sm-4 mt30-xs-max">
                    <div class="i-box">
                        <div class="icons icon-gears">
                        </div>
                        <h3>Automate everything</h3>
                        <p>
                            You can easily automate your liking, commenting and following activities based on specific hashtags and geolocations, as well as unfollow users from different sources.
                        </p>
                    </div>
                </div>
                <div class="col-sm-4 mt30-xs-max">
                    <div class="i-box">
                        <div class="icons icon-rocket">
                        </div>
                        <h3>Start and close</h3>
                        <p>
                            Instagress conveniently works on our servers, which means you can feel free to logout, change accounts, or even close your browser window after you start your Instagress activity.
                        </p>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-4">
                    <div class="i-box">
                        <div class="icons icon-hand">
                        </div>
                        <h3>3 day free trial</h3>
                        <p>
                            We're confident that you'll love our service. Try Instagress free for 3 days and you'll see why Instagrammers continue to use our service.
                        </p>
                    </div>
                </div>
                <div class="col-sm-4 mt30-xs-max">
                    <div class="i-box">
                        <div class="icons icon-joystick">
                        </div>
                        <h3>Full control</h3>
                        <p>
                            We provide tons of filters and customization options to help you increase your Instagram followers, likes and comments for the target audiences that make sense for you.
                        </p>
                    </div>
                </div>
                <div class="col-sm-4 mt30-xs-max">
                    <div class="i-box">
                        <div class="icons icon-brain">
                        </div>
                        <h3>Safe to use</h3>
                        <p>
                            Our one of a kind service will automatically reduce its speed to ensure that your account is safe from hitting Instagram limits and we offer multiple speed settings for more advanced users.
                        </p>
                    </div>
                </div>
            </div>
            <div class="row">
                <div class="col-sm-4">
                    <div class="i-box">
                        <div class="icons icon-clip">
                        </div>
                        <h3>Easy setup</h3>
                        <p>
                            Instagress is easy to set up for one or one hundred accounts. Our team has even included useful tips for each setting and a full how-to guide.
                        </p>
                    </div>
                </div>
                <div class="col-sm-4 mt30-xs-max">
                    <div class="i-box">
                        <div class="icons icon-mobile">
                        </div>
                        <h3>Mobile friendly</h3>
                        <p>
                            We've optimized our website to work seamlessly on all mobile devices. Now you can access and manage your activity whenever you want, wherever you are.
                        </p>
                    </div>
                </div>
                <div class="col-sm-4 mt30-xs-max">
                    <div class="i-box">
                        <div class="icons icon-support">
                        </div>
                        <h3>Customer support</h3>
                        <p>
                            We guarantee that you will receive technical assistance whenever you need it. Feel free to contact us with your questions and our friendly team will get back to you within 24 hours.
                        </p>
                    </div>
                </div>
            </div>

            <div class="dividing-line">
                <hr>
                <div class="dividing-line-icon dashboard"></div>
            </div>
            <div class="row">
                <div class="col-sm-6 col-sm-push-6">
                    <div class="best-features bf-dashboard"></div>
                </div>
                <div class="col-sm-6 col-sm-pull-6 pr50-sm-min">
                    <h3>Dashboard</h3>
                    <h4>Manage multiple accounts</h4>
                    <p>
                        Through our sleek dashboard, you can monitor your activity, check your stats, start or stop all activities simultaneously, transfer time between Instagram accounts and even get a quantity discount. Best of all, you can manage one or hundreds of Instagram accounts in one dashboard.
                    </p>
                    <div class="read-more mt20">
                        <a href="blog/dashboard.html" class="text-large">Get a quick overview of the Dashboard</a>
                    </div>
                </div>
            </div>

            <div class="dividing-line">
                <hr>
                <div class="dividing-line-icon geolocations"></div>
            </div>
            <div class="row">
                <div class="col-sm-6">
                    <div class="best-features bf-geolocations"></div>
                </div>
                <div class="col-sm-6 pl50-sm-min">
                    <h3>Geolocations</h3>
                    <h4>Communicate with your target audience</h4>
                    <p>
                        Geolocation targeting allows you to communicate with people based on the location tags from their Instagram photos. Targeting can be done by country, city, or the most popular places in your area which we suggest for you based on Foursquare.
                    </p>
                    <div class="read-more mt20">
                        <a href="blog/geolocations.html" class="text-large">Learn how to best use Geolocations</a>
                    </div>
                </div>
            </div>

            <div class="dividing-line">
                <hr>
                <div class="dividing-line-icon schedule"></div>
            </div>
            <div class="row">
                <div class="col-sm-6 col-sm-push-6">
                    <div class="best-features bf-schedule"></div>
                </div>
                <div class="col-sm-6 col-sm-pull-6 pr50-sm-min">
                    <h3>Schedule</h3>
                    <h4>Full control of your activity time</h4>
                    <p>
                        The Schedule allows you to plan when your Instagress activity is running, which gives you total control over your account and your time. This function also helps your account to stay safe by ensuring that your activity looks less like a bot and more like a human.
                    </p>
                    <div class="read-more mt20">
                        <a href="blog/schedule.html" class="text-large">Learn how to get the most out of the Schedule</a>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="index-for-whom">
        <div class="container">
            <h2 class="title-big">For Whom</h2>
            <div class="row">
                <div class="col-sm-9 col-md-6 for-whom-cont">
                    <div class="review">
                        <div class="for-whom-cont-icon for-personal"></div>
                        <h3 class="ml50">Personal</h3>
                        <p class="mt10">
                            Instagram gives you a unique opportunity to share your life with people all around the world. With the help of our service you can interact with potential followers 24/7, day in, day out. Turn on your activity while you're sleeping or at work and your flow of followers and likes will never end.
                        </p>
                    </div>
                    <div class="mt50">
                        <div class="for-whom-cont-icon for-business"></div>
                        <h3 class="ml50">Business</h3>
                        <p class="mt10">
                            If you're using Instagram daily to engage with your fans or customers, then you know how much time it takes—especially if you're a social media agency managing hundreds of accounts. Instagress will not only save your time, but will help you to maximize your engagement with Instagram users. The attention your business receives will be neverending.
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <div class="index-how-it-works">
        <div class="container">
            <h2 class="title-big">How It Works</h2>
            <p class="description">Get started in four easy steps</p>
            <div class="row pt10">
                <div class="col-sm-12 col-md-6 step">
                    <div class="col-sm-6 col-sm-push-6">
                        <div class="step-img step-1"></div>
                    </div>
                    <div class="col-sm-6 col-sm-pull-6 mt50">
                        <h3><span class="red-dot mr10">1</span>Sign up</h3>
                        <p>
                            Signing up for Instagress is quick and takes less than a minute. Sign up now to receive your first 3 days for free!
                        </p>
                    </div>
                </div>
                <div class="col-sm-12 col-md-6 mt50-xs-max step">
                    <div class="col-sm-6 col-md-6 col-md-push-6">
                        <div class="step-img step-2"></div>
                    </div>
                    <div class="col-sm-6 col-md-6 col-md-pull-6 mt50">
                        <h3><span class="red-dot mr10">2</span>Add account</h3>
                        <p>
                            Next, add your Instagram accounts using your credentials. Don't worry, we take your privacy and security very seriously.
                        </p>
                    </div>
                </div>
            </div>
            <div class="row mt90-md-min">
                <div class="col-sm-12 col-md-6 mt50-xs-max step">
                    <div class="col-sm-6 push-sm push50">
                        <div class="step-img step-3"></div>
                    </div>
                    <div class="col-sm-6 pull-sm pull50 mt50">
                        <h3><span class="red-dot mr10">3</span>Adjust settings</h3>
                        <p>
                            There are tons of settings that you can customize to specifically target the Instagram accounts that matter to you.
                        </p>
                    </div>
                </div>
                <div class="col-sm-12 col-md-6 mt50-xs-max step">
                    <div class="col-sm-6">
                        <div class="step-img step-4"></div>
                    </div>
                    <div class="col-sm-6 mt50">
                        <h3><span class="red-dot mr10">4</span>Enjoy and relax</h3>
                        <p>
                            You'll be pleasantly surprised by how easy and effective it is. Just push the red button to get started and watch the new followers roll in!
                        </p>
                    </div>
                </div>
            </div>
            <div class="text-align-center mt40">
                <a href="http://www.instagressalternative.com/app/" class="btn btn-plain btn-big btn-main btn-danger btn-fw-xs-max">
                Sign up for free
            </a>
            </div>
        </div>
    </div>

    <div class="index-reviews">
        <div class="container">
            <h2 class="title-big">Reviews</h2>
            <div class="row">
                <div class="col-sm-9 col-sm-offset-3 col-md-6 col-md-offset-6 reviews-cont">
                    <div class="review">
                        <div class="review-icon"></div>
                        <h3>Absolutely awesome service</h3>
                        <p class="mt10">
                            Instagress has really helped me to connect with my audience on Instagram. Before I was spending hours a day liking, following, and commenting on photos. Now, Instagress does it all for me!
                        </p>
                    </div>
                    <div class="review">
                        <div class="review-icon"></div>
                        <h3>Two thumbs up</h3>
                        <p class="mt10">
                            With Instagress I get REAL followers that I can actually interact with. My newsfeed is flooded with "thank you's" from people whose photos I've liked. What is even cooler is that you've liked photos for me that I would've actually liked myself.
                        </p>
                    </div>
                    <div class="review">
                        <div class="review-icon"></div>
                        <h3>You guys are legends</h3>
                        <p class="mt10">
                            I've been with you guys since your Beta version. Today I got a call from a HUGE wellness company asking how I'm so successful on Instagram. They are now going to fly me out to LA to show them how to use Instagress! Thank you so much for creating this service!
                        </p>
                    </div>
                </div>
            </div>
        </div>
    </div>

    <a href="https://instagressalternative.com/blog/tags/video%20tutorials" class="index-small-block">
        <div class="container">
            <div class="small-block-youtube">
                <h4>Video tutorials</h4>
            </div>
        </div>
    </a>

    <div class="index-blog">
        <div class="container">
            <h2 class="title-big">Blog</h2>
            <div class="text-align-center">
                <a href="blog.html" class="text-large">View all posts</a>
            </div>
            <div class="row">

                <div class="col-sm-6">
                    <div class="row blog-wrap">
                        <div class="col-sm-3 col-md-2">
                            <div class="blog-date">
                                <div class="day">15</div>
                                <div class="month">Mar</div>
                                <div class="year">2016</div>
                            </div>

                        </div>
                        <div class="col-sm-9 col-md-10">
                            <div class="blog-post">
                                <h3>
            <a href="https://instagressalternative.com/blog/video-tutorials-2" class="link-inherit">Video tutorials #2</a>
        </h3>

                                <p>We have accidentally found an amazing video tutorial created by one of our customers about all the settings that you can use on Instagress. We are really excited to share it with you! If you still have some questions about the settings, please don't hesitate and...</p>

                                <div class="mt20">
                                    <a href="https://instagressalternative.com/blog/video-tutorials-2">Read more…</a>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>

                <div class="col-sm-6">
                    <div class="row blog-wrap">
                        <div class="col-sm-3 col-md-2">
                            <div class="blog-date">
                                <div class="day">9</div>
                                <div class="month">Feb</div>
                                <div class="year">2016</div>
                            </div>

                        </div>
                        <div class="col-sm-9 col-md-10">
                            <div class="blog-post">
                                <h3>
            <a href="blog/follow-and-unfollow.html" class="link-inherit">Follow &amp; Unfollow at the same time</a>
        </h3>

                                <p>
                                    With our new feature, Follow & Unfollow at the same time, your Activity can automatically switch between following and unfollowing. How does it work? Make sure you have enabled both the Follow and Unfollow actions in your Activity. It’s also very important to choose “Unfollow source...</p>

                                <div class="mt20">
                                    <a href="blog/follow-and-unfollow.html">Read more…</a>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>

            </div>
        </div>
    </div>

    </section>

    @include('modal.init')
@endsection 